...
/**
 * Visible ARCH002 findings (suppressed / shakeable-hidden do not count for the gate).
 * @param {import("@next-architect/core").Diagnostic[]} diagnostics
 */
function arch002Findings(diagnostics) {
  return diagnostics.filter(
    (d) => d.ruleId === RULE_ID && !d.suppressed,
  );
}
...
lines.push(`ARCH002 corpus FP gate (${manifest.cases.length} cases)`);
lines.push("");
...
for (const c of manifest.cases) {
  ...
  // Bonus: AnalysisResult must always include limitations (even empty).
  ...
  const findings = arch002Findings(outcome.result.diagnostics);
  const files =
    findings
      .map((d) => {
  ...
  switch (c.expectation) {
    case "should-report": {
      if (findings.length === 0) {
        verdict = "fail";
        detail = "expected ARCH002 finding, got 0";
        failed = true;
      } else {
