 ...
 */
function arch003Findings(diagnostics) {
...
lines.push(`ARCH003 corpus FP gate (${manifest.cases.length} cases)`);
lines.push("");
...
for (const c of manifest.cases) {
  ...
  const findings = arch003Findings(outcome.result.diagnostics);
  const files =
    findings
      .map((d) => {
  ...
  switch (c.expectation) {
    case "should-report": {
      if (findings.length === 0) {
        verdict = "fail";
        detail = "expected ARCH003 finding, got 0";
        failed = true;
      } else {
    ...
    case "should-not-report": {
      // ARCH003 defaults to error — any finding on these cases is an FP.
      if (findings.length > 0) {
        verdict = "fail";
